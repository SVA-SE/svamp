...
```{r PPNs Anim owner, results='asis', echo=FALSE, eval=FALSE}
...
n_anim_own_rz <- n_anim_own_rz[,c("Ppn","Namn","Adress.1", "Coadress","Telefonnummer.1",
                                  "Mobilnummer",  "Epost",  "buffer", "nearest")]

n_anim_own_rz <- n_anim_own_rz[!duplicated(n_anim_own_rz),]

colnames(n_anim_own_rz) <-   c("Ppn","Namn", "Adress", "Coadress", "Telefon", "Mobil",
...
out4 = NULL
for (p in 1:length(n_anim_own_rz)) {
g4 <- paste0("<br>", "**9a. OWNERS OF PPNs WITHIN BUFFER** ", names(n_anim_own_rz[p]), "km", "<br>", "<br>")
  knit_expanded4 <- paste0("\n```{r results='asis', echo=FALSE}\n\ndatatable(n_anim_own_rz[[", p, "]], filter='top', escape = FALSE, rownames = FALSE, options = list(pageLength = 5))\n\n```")
