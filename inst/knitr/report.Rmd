...
g2 <- paste0("<br>", "<br>", "**LIST OF FARMS WITHIN BUFFER **", names(farms[k]), "km", "<br>", "<br>")
...
##### **LIST AND NUMBER OF ANIMALS PER PPN WITHIN RESTRICTION ZONES** 
<br>
```{r PPNs Anim number, results='asis', echo=FALSE, eval=TRUE}

n_anim_rz <- PPN[PPN$Ppn %in% farms_in_buffers@data$Ppn, ]

n_anim_rz <- n_anim_rz[c("Ppn", 
                           "Species", 
                           "Typ", 
                           "Namn",
                           "Kommun",
...
n_anim_rz[, col_names] <- lapply(n_anim_rz[, col_names] , factor)
 ...
 datatable(n_anim_rz, filter='top', rownames =FALSE,
...
### 9.1 List of PPN records with missing coordinates within postnummers
<br>
...
### 9.2 Table of PPN records with missing coordinates within postnummers
<br>
...
#### 10.2. **JBV DATASET OF CLINICAL RECORDS**
...
urax_match_ppn <- urax[farms_in_buffers$Ppn %in% urax$PPN, ]

# match ppns without coords 
urax_match_ppn_nc <- urax[PPN_pro_km_2$Ppn %in% urax$PPN, ]
