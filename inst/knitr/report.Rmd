...
```{r Outbreak, warning = FALSE, message = FALSE, echo = FALSE, results = 'asis', eval=TRUE}
...
```{r PlotMappro, warning = FALSE, message = FALSE, echo = FALSE, results = 'asis', fig.height = 14, fig.width = 14,eval=FALSE}
...
df <- data.frame("PPN" = outbreak$Ppn,
                  
                 "RefSyst" = c("RT90", "WGS84", "SWEREF99"),
...
df <- df[order(df$PPN),]
colnames(df) <- c("PPN", "Reference system", "X coordinate", "Y coordinate")
...
pop_out <- paste0("PPN: ", outbreak1_wgs@data$Ppn, "<br>",
                  "Species: ", outbreak1_wgs@data[5], "<br>",
                  "Address: ", outbreak1_wgs@data[3])

LeafletMap1 <- leaflet(outbreak1_wgs) %>% addProviderTiles("CartoDB.Positron")

LeafletMap1 %>% addCircles(data = outbreak1_wgs,
                           stroke = FALSE,
                           fill = TRUE,
                           fillColor = "red",
                           fillOpacity = 1,
...
plot(buf_sp10, axes = T, col = rgb(0,1,0, alpha = 1), lty=5, lwd = 2)
plot(buf_sp3, add = T, col = rgb(1,1,0, alpha = 1), lty=3, lwd = 2)
plot(buf_sp1, add = T, col = rgb(1,0,0, alpha = 1), lwd = 2)
...
LeafletMap2 %>%
  ...
  addCircles(data = farms_in10_84,
                   stroke = TRUE,
                   fill = TRUE,
                   fillColor = "green") %>%
  
  addCircles(data = farms_in3_84,
                   stroke = TRUE,
                   fill = TRUE,
                   fillColor = "yellow") %>%
  
  addCircles(data = farms_in1_84,
                   stroke = TRUE,
                   fill = TRUE,
                   fillColor = "red") %>%
...
anim1_DT <- anim1[, c(1,2,11,14,19,26,43)]
  ...
  datatable(anim1_DT,  extensions = 'TableTools',
           options = list(
                    columnDefs = list(
                                    list(className = 'dt-center',
                                         targets = c(0,1,4,5,6))),
                    dom = 'T<"clear">ftip', pageLength = dim(quakes)[1],
                                    tableTools = list(
                                      sSwfPath = copySWF(),
                                      aButtons = list( list(
                                        sExtends = 'collection',
...
if (!all(is.na(over(postnum_miss, buf_sp1)))) {
  ...
  # match postnummer with postnummer of PPN is.na
    PPN_pro1km <- PPN_is.na[PPN_is.na$Postnummer %in% 
...
if (!all(is.na(over(postnum_miss, buf_sp3)))) {
  ...
  PPN_pro3km <- PPN_is.na[PPN_is.na$Postnummer %in% 
...
if (!all(is.na(over(postnum_miss, buf_sp10)))) {
  ...
  PPN_pro10km <- PPN_is.na[PPN_is.na$Postnummer %in% 
...
SVASSS.SJV <- SVASSS.SJV.alarms.data[SVASSS.SJV.alarms.data$PPN %in% alla$Ppn, ]

# dates.svala <- as.Date(svala.data$ANKOMSTDATUM, format = "%d/%m/%Y", origin="01/01/1970")
# svala.data <- svala.data[which(svala.data$PPN_original%in%herds.info()[[4]]),]
# svala.active <- svala.data[which(svala.data$PPN_original%in%herds.info()[[4]]&svala.data$active2==1),]
# svala.passive <- svala.data[which(svala.data$PPN_original%in%herds.info()[[4]]&svala.data$active2==0),]
...
#### 10.2. DATASET OF LAB SAMPLES (SVALA)
